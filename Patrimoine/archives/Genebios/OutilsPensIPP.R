#####################################################################################################
#                         Bibliothèque d'utilitaires généraux PENSIPP
# 
# 
...
# - Toutes les dates sont comptées par rapport a l'année 1900, par exemple annais[i]=90 pour un 
#   individu né en 1990, statut[i,110] pour son statut en 2010. De m�?me, pour les séries temporelles,
...
# Coefficients d'après Duée (2006)
{
  # On calcule l'age courant et la durée depuis la séparation (s'il y a lieu)
  a     <- age[i]
...
# Les probas sont appliquées à la femme
{
  ...
  {
    # age à l'union, duree écoulée et nombre d'enfants de l'union en cours
...
# -> liste_enf : retourne la liste des id des enfants de l'individu i : par défaut l'ensemble
...
# -> Fonction "pyram"
#
# Retourne des efffectifs par age, avec filtrage optionnel. Le second paramètre, également optionnel, est le 
# pas de la pyramide, par exemple pas=5 pour un regroupement quinquennal. Par défaut, le pas est égal 
# est égal à 1
# 
...
# -> function "by_age"
# Calcul de différentes statistiques descriptives univariées pour une variable donnée, ventilées
# par age avec un pas ajustable (pas=1 par défaut) et filtrage éventuel. Par défaut, la statisti-
# que calculée est la moyenne (type="mean"). Les autres options possibles sont "min, "max", "median" 
# et "sd".
#
# Exemples d'appel
